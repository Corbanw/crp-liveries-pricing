...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Utility Functions
    const formatCurrency = (value) => `${value} Robux`;
    ...
    const calculateTotal = () => {
        ...
        if (sidebarTotal) {
            sidebarTotal.innerHTML = `<span class="robux-icon"></span>${formatCurrency(finalTotal)}`;
        }
    ...
    const checkBundleRecommendations = (subtotal, totalItems) => {
        ...
        if (premiumQty >= 1 && newSealQty >= 1 && subtotal > 220) {
            ...
            recommendationText.textContent = `You've added Premium + New Seal. The Pro Pack saves you ${savings} Robux!`;
